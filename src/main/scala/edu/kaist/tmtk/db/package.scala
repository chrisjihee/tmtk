...
package object db {
  ...
  def testMySQL() = test(method, () => {
    for (db <- managed(new MySQL("143.248.48.105/unit", "admin", "admin1", "samples", "i int, name varchar(20), primary key(i)"))) {
      warn(s" + [DB] $db")
  ...
  def testCassandra() = test(method, () => {
    for (db <- managed(new Cassandra("143.248.48.105/unit", "samples", "v int, i int, name text, alias list<text>, primary key(v, i)"))) {
      warn(s" + [DB] $db")
